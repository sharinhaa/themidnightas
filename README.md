...
CRONOGRAMA SEMANAL DO DESENVOLVIMENTO (6 semanas)
semana 1: O foco se dará nas melhorias no código base e estruturação sonora para a dinâmica e estruturação sonora para a mecânica de furtividade. Na programação geral, será implementado o sistema de dt nas movimentações das entidades para que o jogo rode na mesma velocidade em quelquer computador, independente de oscilações no FPS. Será ajustado também os métodos de colisão para que o jogador não apresente trepidação ("stuttering") ao pressionar o personagem diagonalmente contra as estante s. Por fim, pretendemos iniciar o gerenciador de som e criar a lógica de tensão escalonada: uma trilha sonora básica em loop que adiciona canais/camadas de percussão a cada folha coletada. 

semana 2: Tornar o comportamento do inimigo mais assustador e visualmente perceptível, substituindo a detecção puramente invisível por um efeito visual de cone de luz (lanterna) projetado à frente do bibliotecário utilizando superfícies transparentes. Melhorar também o algoritmo de raycasting criado para garantir que a linha de visão seja interrompida precisamente pelas estantes, mas passa sem problemas pelas mesas de estudo conforme especificado no GDD. 

semana 3: Substituir os blocos coloridos provisórios por arte real e animações fluidas. Comutar os círculos geométricos do estudante e do bibliotecário por imagens reais; implementar a lógica de sprite sheets: criar matrizes de corte para que o jogo renderize o pesonagem caminhando em 4 direções cardinais; mudar o visual do sprite do jogador dinamicamente para uma postura "abaixada/agachada" quando a tecla shift estiver pressionada. 
